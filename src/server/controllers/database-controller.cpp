...
namespace controllers {
            ...
            while (succeed && (dbVersion < DB_VERSION)) {
                ...
                if (succeed) {
                    ...
                    if (!updateQuery.exec()) {
                        ...
                        triggerDBError(updateQuery.lastError(), tr("teacher_ could not be updated to current version."));
                    }
                }
                else {
    ...
    bool DatabaseController::deleteGroup(const int groupId, const QString &groupName)
    {
        ...
        if (query.exec()) {
            ...
            if (query.exec())
                ...
                triggerDBError(query.lastError(), tr("Group <b>%1</b> could not be deleted.").arg(groupName));
        }
        else
            triggerDBError(query.lastError(), tr("Group <b>%1</b> groups_user could not be deleted.").arg(groupName));
        return groupDeleted;
    }
        ...
        if (query.exec()) {
            if (removeAllGroupsForUser(userId)) { // delete from group_user_
                ...
                if (query.exec()) {
                    ...
                    triggerDBError(query.lastError(), tr("User <b>%1</b> could not be deleted from user_.").arg(userId));
                }
            }
        }
        else {
            triggerDBError(query.lastError(), tr("User <b>%1</b> result_ could not be deleted from result_.").arg(userId));
        }
        return userDeleted;
    }
    ...
    bool DatabaseController::removeAllGroupsForUser(const int userId)
    {
        ...
        if (query.exec()) {
            ...
            triggerDBError(query.lastError(), tr("User <b>%1</b> could not be deleted from groups_user_.").arg(userId));
        }
